...
function resetResult() {
  ...
  const myNode = document.getElementById('sentiment-output');
  myNode.innerHTML = '';
...
function startRecording() {
  ...
  // document.getElementById('sentiment-output').value= null;
    ...
    if (err) {
      ...
      lambda.invoke(input, function(err, data) {
             if (err) {
              ...
              //  var cleanResult = resultUrl.replaceAll("\\/","");
               resetView();
               console.log("TEST PAYLOAD", resultUrl)

              document.getElementById('sentiment-output').appendChild(
                document.createTextNode(resultUrl)
              ); 
             }
          });
    }
